...
module ExameCrawler
  ...
  def self.get_info(url)
    ...
    city = spans.select{|span| span.text=="Cidade:"}.first.parent.children[3].text  rescue nil
    ...
    {name: name, sector: sector, city: city, phone: phone, site: site, group: group}
  end
  ...
  def self.save_list(file_path)
    ...
    CSV.open(file_path, "wb") do |csv|
      csv << %w(Nome Setor Cidade Telefone Site Grupo)
